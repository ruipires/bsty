...
std::wstring CodePageToUtf8Conterter::cp1252ToUtf16(std::string const &in)
{
    std::wstring result;
...
//                   UErrorCode * err );
    ...
    if(!U_SUCCESS(status))
    {
        spdlog::error("Unable to convert text. error message:{}", u_errorName(status));
        return L"";
    }

    result = std::wstring(buffer, target);
...
{
    u_setDataDirectory(".");
    icu::UnicodeString us(in.data(), in.size(), "cp1252");
